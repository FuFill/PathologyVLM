...
def main() -> int:
    parser = argparse.ArgumentParser(description="Upload a folder as a ClearML Dataset.")
    parser.add_argument("--dataset_project", required=True, help="ClearML dataset project name.")
    ...
    if not folder.exists() or not folder.is_dir():
        print(f"[upload_clearml_dataset] ERROR: folder does not exist: {folder}", file=sys.stderr)
        return 1

    files = [p for p in folder.rglob("*") if p.is_file()]
    if not files:
        print(f"[upload_clearml_dataset] ERROR: folder is empty: {folder}", file=sys.stderr)
        return 1

    try:
        from clearml import Dataset
    except ImportError as exc:
        print(f"[upload_clearml_dataset] ERROR: 'clearml' is not installed: {exc}", file=sys.stderr)
        return 2
